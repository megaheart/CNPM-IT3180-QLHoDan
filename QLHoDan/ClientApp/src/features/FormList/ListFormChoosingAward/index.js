...
import FormChoosingDetail from '../FormChoosingAwardDetail';
...
export default function ListFormAward() {
    const { auth } = useAuth();

    const { data, isLoading, error } = useQuery(['formRewardForChance', auth.username], async () => await formRewardForChance.getAllFormRewardForChance(auth.token));
    console.log(data)
    ...
    // const mutationDelete = useMutation((id) => formRewardForChance.deleteFormRewardForChance(auth.token, id), {
    //     onMutate: () => {
        ...
        {error ? <ErrorData /> :
            isLoading ? <TableSkeleton /> :
                <Fragment >
                    ...
                    />
                    {idForm && <FormChoosingDetail key={idForm} open={openDetail} onClose={setOpenDetail} idForm={idForm} />}
