...
using QLHoDan.Models.HouseholdForms.ChangingResidentInfoForm;
...
{
    [Route("api/forms/ChangingResidentInfo")]
    [ApiController]
    public class ChangingResidentInfoFormsController : ControllerBase
    {
        ...
        public ChangingResidentInfoFormsController(ApplicationDbContext context,
        ...
        // GET: api/forms/ChangingResidentInfo
        /// <summary>
        ///Lấy ra danh sách form xin thay đổi thông tin nhân khẩu. Phạm vi trả về phụ thuộc vào quyền hạn của tài khoản:<br/>
        ...
        [HttpGet()]
        [Authorize]
        public async Task<IActionResult> GetChangingResidentInfoForms([FromQuery] bool? isChecked = null)
        {
            ...
            if (user.Role > 3)
            {
                return Ok(_context.ChangingResidentInfoForm.Include(f => f.Resident)
            ...
            else return Ok(_context.ChangingResidentInfoForm.Include(f => f.Resident)
                                .Where(f => isChecked == null || (f.IsAccepted == true || !string.IsNullOrEmpty(f.NotAcceptedReason)) == isChecked)
                                .Select(f => new FormBriefInfo()
                                {
                                    Id = f.Id,
        ...
        // GET: api/forms/ChangingResidentInfo/5
        /// <summary>
        ///Lấy ra thông tin chi tiết của form thay đổi thông tin nhân khẩu. Phạm vi trả về phụ thuộc vào quyền hạn của tài khoản:<br/>
        ///1. Chủ tịch phường, kế toán: Sẽ trả về form của toàn bộ phường<br/>
        ///2. Tổ trưởng: Sẽ trả về form của chỉ tổ phụ trách<br/>
        ///3. Hộ dân: Sẽ trả về form của chỉ cá nhân tài khoản đó gửi lên
        /// </summary>
        /// <param name="id">
        /// mã định danh của form thay đổi thông tin nhân khẩu
        /// </param>
        [HttpGet("{id}")]
        [Authorize]
        public async Task<IActionResult> GetChangingResidentInfoForm([FromRoute] int id)
        {
            var form = await _context.ChangingResidentInfoForm.Include(f => f.Resident)
            ...
            ChangingResidentInfoFormDetail changingresidentinfoformdetail = new ChangingResidentInfoFormDetail()
            {
                Id = form.Id,
            ...
            return Ok(changingresidentinfoformdetail);
        }

        // POST: api/forms/ChangingResidentInfo
        /// <summary>
        /// Gửi form thay đổi thông tin nhân khẩu
        /// </summary>
        [HttpPost()]
        [Authorize()]
        public async Task<IActionResult> PostChangingResidentInfoForm([FromForm] AddingChangingResidentInfoFormRequestModel model)
        {
            var userId = User.FindFirst(ClaimTypes.NameIdentifier).Value;
            var user = await _userManager.FindByIdAsync(userId);
            if (user == null)
            ...
            ChangingResidentInfoForm f = new ChangingResidentInfoForm()
            {
                Resident = resident,
            ...
            _context.ChangingResidentInfoForm.Add(f);
        ...
        // POST: api/forms/ChangingResidentInfo/accept/5
        /// <summary>
        ///Phê duyệt hoặc từ chối form thay đổi thông tin nhân khẩu. Phạm vi trả về phụ thuộc vào quyền hạn của tài khoản:<br/>
        ...
        public async Task<IActionResult> CheckChangingResidentInfoForm([FromRoute] int id, [FromBody] AcceptingFormRequestModel model)
        {
            var form = await _context.ChangingResidentInfoForm.Include(f => f.Resident).FirstOrDefaultAsync(f => id == f.Id);
            if (form == null)
            {
                return NotFound();
            }
            ...
            _context.ChangingResidentInfoForm.Update(form);
            await _context.SaveChangesAsync();
        ...
        // DELETE: api/forms/ChangingResidentInfo/5
        /// <summary>
        ///Rút lại form thay đổi thông tin nhân khẩu (có thể do thấy sai hay gì đó). 
        ...
        [HttpDelete("{id}")]
        [Authorize()]
        public async Task<IActionResult> DeleteChangingResidentInfoForm([FromRoute] int id)
        {
            var userName = User.FindFirst(ClaimTypes.Name).Value;
            var form = await _context.ChangingResidentInfoForm.FirstOrDefaultAsync(f => id == f.Id && f.Account == userName);
            if (form == null)
            {
                return NotFound();
            }
            ...
            _context.ChangingResidentInfoForm.Remove(form);
            await _context.SaveChangesAsync();
